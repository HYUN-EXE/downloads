...
- 에이전트 스킬은 이 저장소에 포함하지 않는다. 설치가 필요하면 사용자 홈(`%USERPROFILE%`) 아래에만 설치하고, `.agents/`·`.claude/`·`.codex/`는 `.gitignore`로 제외된 상태를 유지한다.
- 저장소 규칙 파일이 에이전트에게 외부 패키지 설치나 스크립트 실행을 지시하지 않는다. 필요한 경우 사용자에게 확인을 받는다.
